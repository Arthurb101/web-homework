...
function TransactionsQuery (props) {
  ...
  function convertIntoCurrency (amount) {
    ...
    // currency convertion is very complicated and if this was on a live site I would communicate with people on how the company
    // handles currency convertion but this is just a test app, so keeping it simple
    return ((amount / 100) * props.currencyConvertionRate).toFixed(2)
  }
...
TransactionsQuery.propTypes = {
  currencyConvertionRate: PropTypes.number
}
...
class Home extends React.Component {
  ...
  currencyChange (event) {
    ...
    axios(api)
      .then(
        (result) => {
          this.setState({ currencyConvertionRate: result.data.rates[currency] })
        }
      )
  }
    ...
    return (
      <div className='container' style={{ textAlign: 'center' }}>
        ...
        <table className='table is-fullwidth is-striped'>
          ...
          <tbody>
            <TransactionsQuery currencyConvertionRate={this.state.currencyConvertionRate} />
          </tbody>
        </table>
      </div>
    )
